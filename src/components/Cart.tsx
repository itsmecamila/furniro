...
export default function Cart() {
  ...
  const subtotal = products.reduce(
    (prev, curr) => (curr.price - curr.price * curr.discount) * curr.quantity + prev, 0
  );
  ...
  return (
    <>
      <button onClick={open}>
        <img src="https://furniro-bucket.s3.us-east-2.amazonaws.com/icons/shopping-cart.svg" alt="Cart" />
      </button>

      <dialog
        ref={modalRef}
        className="backdrop:bg-black/10 mr-0 mt-0"
      >
        <div className="px-8 pt-8 pb-6">
          <header className="flex items-center justify-between gap-36 pb-6 mb-10 border-b border-[#D9D9D9]">
          ...
          <ul className="flex flex-col gap-5 pb-6">
            { products.map((product) => (
              ...
              >
                <img src={product.cover} className="size-20 bg-black rounded-md" />
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-lg">
                    ...
                    <strong className="text-[#B88E2F]">
                      {currency.format(product.price - (product.price * product.discount))}
                    </strong>
                  </div>
                </div>

                <button onClick={() => dispatch(removeProduct(product.id, 1))}>
                  <img src="https://furniro-bucket.s3.us-east-2.amazonaws.com/icons/remove-item.svg" alt="Remove Item" />
                </button>
              </li>
            )) }
          </ul>
          ...
          <div className="flex items-center justify-between">
            ...
            <strong className="font-bold text-[#B88E2F]">{currency.format(subtotal)}</strong>
          </div>
        </div>
